...
class WelcomeController < ApplicationController
  ...
  def index
    ...
    all_buses.each do |bus|
      ...
      records.each do |start_time, end_time|
        ...
        end_time = Date.today.end_of_month if end_time > Date.today.end_of_month
    ...
    all_buses.each do |bus|
      ...
      records.each do |start_time, end_time|
        ...
        end_time = Date.today.end_of_year if end_time > Date.today.end_of_year
    ...
    services_previous_30_days = current_user.company.services.where('services.fecha >= :start AND services.fecha <= :end', { start: Date.today - 2.months, end: Date.today - 1.month }).count
    ...
    current_user.company.buses.each do |bus|
      ...
      unless correctivo_checkup_needed && services_after_correctivo_checkup.any?
        next
      end
  ...
  def reporte
    ...
    @months = Array(1..12)
    @month = if @year.to_s == Date.current.year.to_s
