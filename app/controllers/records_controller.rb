...
class RecordsController < ApplicationController
  before_action :set_record, only: %i[client itinerary trip bus price payments pdf show edit update destroy]
  ...
  def pizarron
    ...
    start_time = params[:start] ? params[:start].to_time : Time.zone.now
    end_time = params[:end] ? params[:end].to_time : Time.zone.now
    ...
    current_user.company.buses.each do |bus|
      ...
      next unless (preventivo_checkup_needed && services_after_preventivo_checkup.any?) ||
                  (correctivo_checkup_needed && services_after_correctivo_checkup.any?)
